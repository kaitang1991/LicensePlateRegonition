   ...
   public class LicensePlateDetector : DisposableObject
   {
         ...
         List<RotatedRect> detectedLicensePlateRegionList)
      {
         List<String> licenses = new List<String>();
            ...
            using (VectorOfVectorOfPoint temps = new VectorOfVectorOfPoint())
            using (VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint())
            
            {
                CvInvoke.CvtColor(img, gray, ColorConversion.Bgr2Gray);
                CvInvoke.Sobel(gray, sobel, DepthType.Cv8U, 2, 0);
                ...
                CvInvoke.Dilate(threshold, dilate_erode, null, new Point(1, 0), 5, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Erode(dilate_erode, dilate_erode, null, new Point(1, 0), 4, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Dilate(dilate_erode, dilate_erode, null, new Point(1, 0), 1, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Erode(dilate_erode, dilate_erode, null, new Point(0, 1), 3, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Dilate(dilate_erode, dilate_erode, null, new Point(0, 1), 3, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                ...
                FindLicensePlate(contours, hierachy, 0, gray, dilate_erode, licensePlateImagesList, filteredLicensePlateImagesList, detectedLicensePlateRegionList, licenses);
            }
            return licenses;
      }
      ...
      private void FindLicensePlate(
         VectorOfVectorOfPoint contours, int[,] hierachy, int idx, IInputArray gray, IInputArray sobel,
         ...
         for (; idx >= 0;  idx = hierachy[idx,0])
         {
            ...
            using (VectorOfPoint contour = contours[idx])
            {
               if (CvInvoke.ContourArea(contour) > 800)
               {
                  ...
                  //   FindLicensePlate(contours, hierachy, hierachy[idx, 2], gray, sobel, licensePlateImagesList,
      ...
      private static UMat FilterPlate(UMat plate)
      {
         ...
         CvInvoke.Threshold(plate, thresh, 120, 255, ThresholdType.Otsu);
         //Image<Gray, Byte> thresh = plate.ThresholdBinaryInv(new Gray(120), new Gray(255));

         Size plateSize = plate.Size;
         using (Mat plateMask = new Mat(plateSize.Height, plateSize.Width, DepthType.Cv8U, 1))
         using (Mat plateSobel = new Mat())
         using (VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint())
         {
            plateMask.SetTo(new MCvScalar(255.0));
            CvInvoke.Sobel(plate, plateSobel, DepthType.Cv8U, 2, 0);
            CvInvoke.FindContours(plateSobel, contours, null, RetrType.External, ChainApproxMethod.ChainApproxSimple);
