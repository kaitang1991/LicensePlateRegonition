...
namespace LicensePlateRecognition
{
   ...
   public class LicensePlateDetector : DisposableObject
   {
      ...
      public LicensePlateDetector(String dataPath)
      {
         ...
         _ocr.SetVariable("tessedit_char_whitelist", "ABCDEFGHJKLMNPQRSTUVWXYZ1234567890");
      }
         ...
         List<RotatedRect> detectedLicensePlateRegionList)
      {
         List<String> licenses = new List<String>();
            ...
            using (Mat save = new Mat())
            using (Mat threshold = new Mat())
            using (Mat dilate_erode = new Mat())
            using (VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint())
            {
                CvInvoke.BitwiseAnd(img, img, save);
                Mat[] split;
                split = save.Split();
                //CvInvoke.Imshow("split", save);
                //CvInvoke.Imshow("0", split[0]);
                //CvInvoke.Imshow("1", split[1]);
                //CvInvoke.Imshow("2", split[2]);
                //CvInvoke.Imshow("gray",gray);

                CvInvoke.AddWeighted(split[1], 0.2, split[2], 0.8, 0, gray);
                CvInvoke.PyrDown(gray, gray);
                CvInvoke.Imshow("temp",gray);
                             
                CvInvoke.BilateralFilter(gray, temp, 8, 80, 80);
                //CvInvoke.CvtColor(temp, gray, ColorConversion.Bgr2Gray);
                CvInvoke.Sobel(temp, sobel, DepthType.Cv8U, 1, 0);
                //CvInvoke.Imshow("sobel", sobel);
                CvInvoke.Threshold(sobel, threshold, 0, 255, ThresholdType.Otsu);
                //CvInvoke.Imshow("threshold", threshold);

                CvInvoke.Dilate(threshold, dilate_erode, null, new Point(1, 0), 14, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Erode(dilate_erode, dilate_erode, null, new Point(1, 0), 17, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Dilate(dilate_erode, dilate_erode, null, new Point(1, 0), 3, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Erode(dilate_erode, dilate_erode, null, new Point(0, 1), 5, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Dilate(dilate_erode, dilate_erode, null, new Point(0, 1), 5, BorderType.Constant, CvInvoke.MorphologyDefaultBorderValue);
                CvInvoke.Imshow("dilate", dilate_erode);

                int[,] hierachy = CvInvoke.FindContourTree(dilate_erode, contours, ChainApproxMethod.ChainApproxSimple);

                FindLicensePlate(contours, hierachy, 0, temp, dilate_erode, licensePlateImagesList, filteredLicensePlateImagesList, detectedLicensePlateRegionList, licenses);
            }
         return licenses;
      }
         ...
         List<String> licenses)
      {
         for (; idx >= 0;  idx = hierachy[idx,0])
         {
            ...
            {
               if (CvInvoke.ContourArea(contour) > 800)
               {
      ...
      private static UMat FilterPlate(UMat plate)
      {
         ...
         using (VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint())
         {
            plateMask.SetTo(new MCvScalar(0));
                CvInvoke.Canny(plate, plateCanny, 100, 30);
                //CvInvoke.GaussianBlur(plateCanny, plateCanny, new Size(3, 3), 0);
                //CvInvoke.Imshow("platecanny", plateCanny);
